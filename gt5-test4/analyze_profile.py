#!/usr/bin/env python3
"""Validate GT5 Test 4 stack profiles; values are cumulative observed watermarks."""
import argparse
import csv
import json
...
def parse_profile(text):
    ...
    for line_no, original in enumerate(text.splitlines(), 1):
        ...
        if fields[:3] == ["G4", "M", "version"]:
            current = {"index": len(sessions) + 1, "version": fields[3:], "operations": [],
                       "stacks": [], "trace": [], "verdicts": [], "metadata": {}, "errors": []}
        ...
        try:
            ...
            if kind == "M":
                ...
                current["trace"].append(("S", fields[2]))
                ...
                current["trace"].append(("O", fields[2]))
    ...
    for session in sessions:
        ...
        if session["metadata"].get("heap_pool_config") != ["4096"]:
            problems.append("unexpected configured heap pool size")
        if session["metadata"].get("caps") != ["1", "0"]:
            problems.append("unexpected GT5 capabilities")
        ...
        if session["trace"] != [("S", "before_operations")] + [("O", "sync_timeout")] * 3 + [("S", "sync_timeout")]:
            problems.append("stack/operation interleaving is invalid")
        previous = {}
