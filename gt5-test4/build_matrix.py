...
def validate_config(variant, config, baseline_config=None):
    ...
    expected = {"CONFIG_T4_" + variant.upper(): "y", "CONFIG_SERIAL": "y",
                ...
                "CONFIG_HEAP_MEM_POOL_SIZE": "4096", "CONFIG_SMP": "n", "CONFIG_SHELL": "n",
    ...
    if variant != "baseline":
        expected.update(CONFIG_BIOMETRICS_GT5X="y")
...
def verify_sources(zephyr, reference):
    required = json.loads(Path(reference).read_text())["required_source_sha256"]
    actual = {}
    for relative, expected in required.items():
        path = Path(zephyr) / relative
        if not path.is_file():
            raise RuntimeError(f"Required source missing: {relative}")
        actual[relative] = hashlib.sha256(path.read_bytes()).hexdigest()
        if actual[relative] != expected:
            raise RuntimeError(f"Source differs from verified GT5 Tests 1/3: {relative}. "
                               "Retain the evidence and inspect the change; no sources were modified.")
    return actual
...
def main():
    ...
    ap.add_argument("--board", required=True, help="same full board target as Tests 1/3")
    ap.add_argument("--overlay", type=Path, default=Path("app.overlay"))
    ap.add_argument("--tag", default="gt5-t4-01", help="new identifier; prevents overwriting")
    ...
    if not overlay.is_file():
        ap.error("Copy the working GT5 overlay to app.overlay first")
    ...
    metadata = {"sensor": "gt5x", "harness_version": "1.0", "collector_version": "1.3", "board": args.board, "tag": args.tag,
    ...
    try:
        ...
        metadata["source_verification"] = verify_sources(zephyr, app / "reference_sha256.json")
        ...
        for relative in ("prj.conf", "Kconfig", "CMakeLists.txt", "src/main.c", "README.md",
                         ...
                         "reference_sha256.json", "heap_accounting.json"):
        ...
        for variant in VARIANTS:
            ...
            if variant == "baseline" and details["driver_data_objects"]:
                raise RuntimeError("Baseline unexpectedly contains GT5 driver data")
            if variant != "baseline" and len(details["driver_data_objects"]) != 1:
                raise RuntimeError("Expected exactly one GT5 instance; inspect overlay/symbols")
