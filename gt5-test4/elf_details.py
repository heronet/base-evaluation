...
def inspect_elf(path, csv_path):
    with Path(path).open("rb") as f:
        ...
        for symbol in symtab.iter_symbols():
            if symbol.name.startswith("gt5x_"):
        ...
        if has_dwarf:
            ...
            for cu in dwarf.iter_CUs():
                for die in cu.iter_DIEs():
                    ...
                    if die.tag != "DW_TAG_structure_type" or name is None or name.value != b"gt5x_data":
                        continue
                    size = type_size(die)
                    if size is None:
                        continue
        ...
        data_objects = [s for s in symbols if s["name"].startswith("gt5x_data_") and s["type"] == "STT_OBJECT"]
        ...
        return {"elf": str(path), "machine": elf["e_machine"], "class_bits": elf.elfclass,
                ...
                "driver_data_objects": data_objects, "gt5x_data_layouts": layouts,
                "accounting_note": "Each gt5x_data object includes inline command/response packets, synchronization state and the template_buf pointer. "
                "The pointed-to heap payload is not part of that object. GT5 has no private worker stack. "
                "See heap_accounting.json for source-derived allocation requests, not measured allocator usage. "
                "Do not add heap payloads to native RAM totals when their pool is already included. "
