#!/usr/bin/env python3
"""Build GT5 Test 1 and retain source/configuration evidence; never flash."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    p.add_argument('--tag', default='gt5-t1-01')
    a = p.parse_args()
    ...
    metadata = {'test': 'GT5 Test 1', 'version': '1.0', 'board': a.board,
    ...
    try:
        ...
        for rel in ('CMakeLists.txt', 'prj.conf', 'app.overlay', 'src/main.c',
                    'README.md', 'build.py'):
        ...
        cache = (out / 'CMakeCache.txt').read_text().splitlines()
        source_entries = [line.split('=', 1)[1] for line in cache
                          if line.startswith('ZEPHYR_BASE:') and '=' in line]
        if not source_entries or Path(source_entries[-1]).resolve() != source:
            raise RuntimeError('CMake selected an unexpected Zephyr source checkout')
