...
VARIANTS = ("baseline", "idle", "sync", "async", "profile")
...
def validate_config(variant, config, baseline_config=None):
    ...
    if variant != "baseline":
        expected.update(CONFIG_BIOMETRICS_AI10="y", CONFIG_BIOMETRICS_AI10_THREAD_PRIORITY="5",
                        CONFIG_BIOMETRICS_AI10_STACK_SIZE="2048")
...
def main():
    ...
    ap.add_argument("--tag", default="ai10-t4-01", help="new identifier; prevents overwriting")
    ...
    if not overlay.is_file():
        ap.error("Copy the working AI10 overlay to app.overlay first")
    ...
    metadata = {"harness_version": "1.0", "collector_version": "1.3", "board": args.board, "tag": args.tag,
    ...
    try:
        ...
        for variant in VARIANTS:
            ...
            if variant == "baseline" and details["driver_data_objects"]:
                raise RuntimeError("Baseline unexpectedly contains AI10 driver data")
            if variant != "baseline" and len(details["driver_data_objects"]) != 1:
                raise RuntimeError("Expected exactly one AI10 instance; inspect overlay/symbols")
