...
def inspect_elf(path, csv_path):
    with Path(path).open("rb") as f:
        ...
        for symbol in symtab.iter_symbols():
            if symbol.name.startswith("ai10_"):
        ...
        if has_dwarf:
            ...
            for cu in dwarf.iter_CUs():
                for die in cu.iter_DIEs():
                    ...
                    if die.tag != "DW_TAG_structure_type" or name is None or name.value != b"ai10_data":
                        continue
                    size = type_size(die)
                    if size is None:
                        continue
        ...
        data_objects = [s for s in symbols if s["name"].startswith("ai10_data_") and s["type"] == "STT_OBJECT"]
        ...
        return {"elf": str(path), "machine": elf["e_machine"], "class_bits": elf.elfclass,
                ...
                "driver_data_objects": data_objects, "ai10_data_layouts": layouts,
                "accounting_note": "Each ai10_data object ALREADY includes its embedded worker stack. "
                "DWARF member sizes describe reserved storage, not observed stack use. "
