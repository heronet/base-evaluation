 ...
 * BASE ZFM Test 3 v1.0: timer-release scheduling interference, one CPU.
...
#error "Use the ZFM overlay with alias biometrics."
#endif
...
{
	shell_print(sh, "Z3,M,%u,%u,%s,%lld,%lld", run, cell, name,
...
{
	shell_print(sh, "Z3,V,%u,%u,%s,%s,%d", run, cell, name, ok ? "PASS" : "FAIL", rc);
}
		   ...
		   enum mode mode, int priority, unsigned int load)
{
	...
	for (unsigned int i = 0; i < JOBS; ++i) {
		...
		shell_print(sh, "Z3,J,%u,%u,%u,%u,%u,%u", run, cell, i,
...
static int cmd_info(const struct shell *sh, size_t argc, char **argv)
{
	...
	if (ret) { return ret; }
	shell_print(sh, "BASE ZFM T3 v1.0; device %s; no samples; no database changes.", bio->name);
...
SHELL_CMD_REGISTER(t3, &t3_commands, "ZFM scheduling evaluation", NULL);
