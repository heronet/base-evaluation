 ...
 * BASE ZFM Test 4 v1.0. Static-build comparison and separate stack profiling.
 ...
 * ZFM has no private worker thread. This scans the application stack used by
...
{
	int ret = 0;
	printk("Z4,M,version,1,0\n");
#if defined(CONFIG_T4_BASELINE)
	printk("Z4,M,variant,baseline\n");
#else
