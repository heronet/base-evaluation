#!/usr/bin/env python3
"""BASE T3 v1.1. Preserve invalid/incomplete runs; never overwrite output.
...
MASK = (1 << 32) - 1
MODES = ("idle", "sync", "async")
RUN_META = {"version", "clock", "period_ticks", "jobs_period_ms",
            "victim_calibration", "load_calibration", "priorities"}
CELL_META = {"condition", "load_percent", "window_cycles", "job_counts",
             "background_counts", "bio_counts", "bio_window_completed"}
...
def parse(text):
    ...
    if not runs:
        errors.append("No T3 runs found")
...
def calculate(runs, parse_errors):
    ...
    for (session, run), data in runs.items():
        ...
        if set(data["cells"]) != set(range(1, 13)):
            errors.append("expected exactly cells 1..12")
        ...
        for cell, entry in sorted(data["cells"].items()):
            try:
                ...
                if meta["version"] not in ([1, 0], [1, 1]) or meta["jobs_period_ms"] != [300, 10]:
                ...
                if mode not in range(3) or priority not in (2, 8) or load not in (0, 50) or lp != 4:
                ...
                group = (cell - 1) // 3
                expected = (((cell - 1) % 3 + run - 1 + group) % 3,
                ...
                if meta["version"] == [1, 1] and cm.get("processed_jobs") != [300, 0]:
...
def make_plots(directory, summaries, jobs):
    ...
    colors = {"idle": "#555555", "sync": "#0072B2", "async": "#D55E00"}
