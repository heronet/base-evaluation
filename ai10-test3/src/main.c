 ...
 * BASE Test 3 v1.1: timer-release scheduling interference, AI10, one CPU.
...
#endif
BUILD_ASSERT(CONFIG_BIOMETRICS_AI10_THREAD_PRIORITY == BIO_PRIORITY);
BUILD_ASSERT(CONFIG_NUM_PREEMPT_PRIORITIES > 8);

enum mode { IDLE, SYNC, ASYNC };
static const char *const mode_names[] = { "idle", "sync", "async" };
...
static atomic_t event_count, stopped_count, outcome_count;
...
{
	if (dev != bio || data != NULL || k_is_in_isr()) { atomic_set(&bio_error, -EPROTO); }
	unsigned int index = (unsigned int)atomic_inc(&event_count);
	if (e->type == BIOMETRIC_EVENT_NO_MATCH) {
		atomic_inc(&outcome_count);
		if (index != 0U || e->status != -ETIMEDOUT) {
			atomic_set(&bio_error, e->status ? e->status : -EPROTO);
		}
	} else if (e->type == BIOMETRIC_EVENT_STOPPED) {
		atomic_inc(&stopped_count);
		if (index != 1U || e->status != 0) {
			atomic_set(&bio_error, e->status ? e->status : -EPROTO);
		}
		k_sem_give(&terminal);
	} else {
		/* A presented sample changes the workload: retain and reject that cell. */
		atomic_set(&bio_error, e->status ? e->status : -EPROTO);
	}
}
	...
	for (;;) {
		...
		while (atomic_get(&bio_running)) {
			...
			if (current_mode == SYNC) {
				...
				atomic_clear(&event_count);
				atomic_clear(&stopped_count);
				atomic_clear(&outcome_count);
				k_sem_reset(&terminal);
				ret = biometric_match_async(bio, BIOMETRIC_MATCH_IDENTIFY, 0,
							    false, K_SECONDS(1));
				if (ret == 0) { ret = k_sem_take(&terminal, K_SECONDS(12)); }
				if (ret == 0) { ret = callback_idle(); }
				if (ret == 0 && (atomic_get(&event_count) != 2 ||
					atomic_get(&stopped_count) != 1 ||
					atomic_get(&outcome_count) != 1)) { ret = -EPROTO; }
			}
...
{
	shell_print(sh, "T3,M,%u,%u,%s,%lld,%lld", run, cell, name,
...
{
	shell_print(sh, "T3,V,%u,%u,%s,%s,%d", run, cell, name, ok ? "PASS" : "FAIL", rc);
}
		   ...
		   enum mode mode, int priority, unsigned int load)
{
	...
	for (unsigned int i = 0; i < JOBS; ++i) {
		...
		shell_print(sh, "T3,J,%u,%u,%u,%u,%u,%u", run, cell, i,
...
static int cmd_info(const struct shell *sh, size_t argc, char **argv)
{
	...
	if (ret) { return ret; }
	shell_print(sh, "BASE T3 v1.1; device %s; no samples; no database changes.", bio->name);
...
static int cmd_run(const struct shell *sh, size_t argc, char **argv)
{
	...
	if (*argv[1] == '\0' || *end != '\0' || parsed == 0 || parsed > 1000000UL) {
		return -EINVAL;
	}
	...
	if (!ret && !(caps.async_operations & BIOMETRIC_ASYNC_IDENTIFY)) { ret = -ENOTSUP; }
	...
	if (ret == 0) {
		...
		for (unsigned int group = 0; group < 4 && ret == 0; ++group) {
			...
			for (unsigned int m = 0; m < 3 && ret == 0; ++m) {
				enum mode mode = (enum mode)((m + run - 1U + group) % 3U);
...
SHELL_STATIC_SUBCMD_SET_CREATE(t3_commands,
	...
	SHELL_CMD_ARG(run, NULL, "Run all 12 conditions: t3 run <positive-run-id>.", cmd_run, 2, 0),
	SHELL_SUBCMD_SET_END);
SHELL_CMD_REGISTER(t3, &t3_commands, "BASE scheduling evaluation", NULL);
